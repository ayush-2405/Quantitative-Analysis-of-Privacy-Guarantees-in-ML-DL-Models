...
class DPCompatibleMLP(nn.Module): 
    def __init__(self, input_dim, num_classes=2): 
        super().__init__() 
        self.fc1 = nn.Linear(input_dim, 512)  # Increased capacity 
...
def train_dp_sgd(X, y, epochs=10, batch_size=128, lr=0.01,  
    ...
    test_ds = CreditDataset(X_te, y_te) 
...
shuffle=True) 
    ...
    input_dim = X.shape[1] 
    model = DPCompatibleMLP(input_dim) 
    optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.9) 
    criterion = nn.CrossEntropyLoss() 
...
class ReconstructionAttack: 
    ...
    def attack(self, target_labels, num_samples=None, epochs=1000, lr=0.1): 
        # Automatically match num_samples to target_labels if not specified 
        if num_samples is None: 
            num_samples = len(target_labels) 
             
        synthetic_data = torch.randn( 
            (num_samples, self.input_dim),  
            requires_grad=True 
        ) 
...
def main(): 
    ...
    dp_model = train_dp_sgd(X, y, epochs=10, batch_size=64,  
                           lr=0.001, max_grad_norm=1.2, 
                           noise_multiplier=1.5, delta=1e-5) 
     
    print("\nTraining Non-DP Model...") 
    non_dp_model = train_non_dp_sgd(X, y, epochs=15, batch_size=128, 
lr=0.005) 
 
    # Reconstruction attack setup 
    attack_data = X[:200]  # Use more samples for better reconstruction 
    attack_labels = y[:200] 
    ...
    dp_recon = dp_attacker.attack(torch.tensor(attack_labels), epochs=2000) 
    non_dp_recon = non_dp_attacker.attack(torch.tensor(attack_labels), 
epochs=2000) 
    attack_data = X[:200] 
    attack_labels = y[:200] 
 
    # To (add num_samples parameter): 
    dp_recon = dp_attacker.attack( 
        torch.tensor(attack_labels),  
        num_samples=len(attack_labels),  # Match number of samples to 
labels 
        epochs=2000 
    ) 
    non_dp_recon = non_dp_attacker.attack( 
        torch.tensor(attack_labels), 
        num_samples=len(attack_labels),  
        epochs=2000 
    ) 
